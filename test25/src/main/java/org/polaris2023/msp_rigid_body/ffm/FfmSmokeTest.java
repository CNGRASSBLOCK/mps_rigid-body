...
public final class FfmSmokeTest {
    ...
    private static final MemoryLayout VEC3 = MemoryLayout.structLayout(
            ValueLayout.JAVA_DOUBLE.withName("x"),
            ValueLayout.JAVA_DOUBLE.withName("y"),
            ValueLayout.JAVA_DOUBLE.withName("z"));
    private static final long VEC3_X = VEC3.byteOffset(MemoryLayout.PathElement.groupElement("x"));
    private static final long VEC3_Y = VEC3.byteOffset(MemoryLayout.PathElement.groupElement("y"));
    private static final long VEC3_Z = VEC3.byteOffset(MemoryLayout.PathElement.groupElement("z"));
    ...
    public static void main(String[] args) throws Throwable {
        ...
        try (Arena arena = Arena.ofShared()) {
            ...
            if (world.equals(MemorySegment.NULL)) {
                throw new AssertionError("world_create returned null");
            }

            try {
                MemorySegment outGravity = arena.allocate(VEC3);
                api.rcWorldGetGravityOut.invokeExact(world, outGravity);
                assertClose(-9.81, outGravity.get(ValueLayout.JAVA_DOUBLE, VEC3_Y), "initial gravity y");

                api.rcWorldSetGravity.invokeExact(world, vec3(arena, 1.0, 2.0, 3.0));
                api.rcWorldGetGravityOut.invokeExact(world, outGravity);
                assertClose(1.0, outGravity.get(ValueLayout.JAVA_DOUBLE, VEC3_X), "gravity x");
                assertClose(2.0, outGravity.get(ValueLayout.JAVA_DOUBLE, VEC3_Y), "gravity y");
                assertClose(3.0, outGravity.get(ValueLayout.JAVA_DOUBLE, VEC3_Z), "gravity z");

                MemorySegment builder = (MemorySegment) api.rcRigidBodyBuilderCreate.invokeExact(0);
                if (builder.equals(MemorySegment.NULL)) {
                    throw new AssertionError("rigid_body_builder_create returned null");
                }
                ...
                try {
                    ...
                    MemorySegment outTranslation = arena.allocate(VEC3);
                    api.rcRigidBodyGetTranslationOut.invokeExact(world, body, outTranslation);
                    assertClose(4.0, outTranslation.get(ValueLayout.JAVA_DOUBLE, VEC3_X), "body translation x");
                    assertClose(5.0, outTranslation.get(ValueLayout.JAVA_DOUBLE, VEC3_Y), "body translation y");
                    assertClose(6.0, outTranslation.get(ValueLayout.JAVA_DOUBLE, VEC3_Z), "body translation z");
    ...
    private static final class NativeApi {
        ...
        private NativeApi(Path library, Arena arena) {
            ...
            rcAbiVersion = downcall("abi_version", FunctionDescriptor.of(ValueLayout.JAVA_INT));
            rcWorldCreate = downcall("world_create", FunctionDescriptor.of(ValueLayout.ADDRESS, VEC3));
            rcWorldDestroy = downcall("world_destroy", FunctionDescriptor.ofVoid(ValueLayout.ADDRESS));
            rcWorldStep = downcall("world_step", FunctionDescriptor.ofVoid(ValueLayout.ADDRESS, ValueLayout.JAVA_DOUBLE));
            rcWorldSetGravity = downcall("world_set_gravity", FunctionDescriptor.ofVoid(ValueLayout.ADDRESS, VEC3));
            rcWorldGetGravityOut = downcall("world_get_gravity_out", FunctionDescriptor.ofVoid(ValueLayout.ADDRESS, ValueLayout.ADDRESS));
            rcRigidBodyBuilderCreate = downcall("rigid_body_builder_create", FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.JAVA_INT));
            rcRigidBodyBuilderDestroy = downcall("rigid_body_builder_destroy", FunctionDescriptor.ofVoid(ValueLayout.ADDRESS));
            rcRigidBodyBuilderSetTranslation = downcall("rigid_body_builder_set_translation", FunctionDescriptor.ofVoid(ValueLayout.ADDRESS, VEC3));
            rcWorldInsertRigidBody = downcall("world_insert_rigid_body", FunctionDescriptor.of(ValueLayout.JAVA_LONG, ValueLayout.ADDRESS, ValueLayout.ADDRESS));
            rcRigidBodyGetTranslationOut = downcall("rigid_body_get_translation_out", FunctionDescriptor.ofVoid(ValueLayout.ADDRESS, ValueLayout.JAVA_LONG, ValueLayout.ADDRESS));
        }
